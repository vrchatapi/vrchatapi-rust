...
pub async fn download_file_version(
    ...
    file_id: &str,
    version_id: i32,
) -> Result<std::path::PathBuf, Error<DownloadFileVersionError>> {
    let local_var_configuration = configuration;
...
pub async fn get_admin_asset_bundle(
    ...
    let local_var_uri_str = format!(
        ...
        local_var_configuration.base_path,
        adminAssetBundleId = crate::apis::urlencode(admin_asset_bundle_id)
    );
    let mut local_var_req_builder =
        local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetAdminAssetBundleError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
...
pub async fn upload_image(
    ...
    if let Some(local_var_param_value) = frames {
        local_var_form = local_var_form.text("frames", local_var_param_value.to_string());
    }
    if let Some(local_var_param_value) = frames_over_time {
        local_var_form = local_var_form.text("framesOverTime", local_var_param_value.to_string());
    }
